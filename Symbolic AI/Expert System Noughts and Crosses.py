# 03/05/21 - 4/05/21 This is an early program I wrote when I was 13
# it is written sloppily, spamming continue statements, use of bare except, "null", hardcoded check of win/lose state, board representation etc.
# HOWEVER it plays optimally following a set of rules (an expert system)
# very close to Newell and Simon's 1972 tic-tac-toe program.
# It checks for winning moves first, then block opponent wins, then look for forks and follow strategy.
# This allows it to be more efficient than minimax and does not enumerate each possible state.
# All comments below were written in 2021
...
while True:
    ...
    row3[0] = "x"
    print(str(row1) + "\n" + str(row2) +"\n" + str(row3))
    print("input type: c1 r2, would put an O on column 1, row 2")
    over = "null"
    gameIteration = 0
    while over=="null":
            ...
            try:
                if move[4] == "1": 
                    if row1[int(move[1])-1] == "x" or row1[int(move[1])-1] == "o":
                        ...
                        move = "null"
                if move[4] == "2": 
                    if row2[int(move[1]-1)] == "x" or row2[int(move[1])-1] == "o":
        ...
        if row1[0] == "o" and row3[0] == "o" and row2[0] == "-":
            ...
            continue
        #Starting Positions
        #when the player places adjacentent to the AI
